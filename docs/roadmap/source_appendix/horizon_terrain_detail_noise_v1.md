...
| `baseAmplitude`（陆地） | **~17 m（实测校准，Honshu 山地）**，场景化 10–30 m | 噪声最大偏移量（= 80m 周期 octave 的峰值振幅）。**已由 GEBCO 2026 日本子集变程分析实测校准**（见下文"实测校准"），原猜测 15–45 m 偏宽，实测集中在低段 |
| `baseAmplitude`（海底） | **~2 m（实测校准，西太平洋深海）**，场景化 2–6 m | 海底细节比陆地小得多（沙波纹、小型海丘）。实测 1.9 m，低于原猜测 3–12 m 的低端，故下调 |
...
## 实测校准（基于 GEBCO 2026 日本子集）

> **为什么需要校准**：上文 `baseAmplitude` 是"合理起始值"的猜测。Phase 0.5 末用项目已有的真实 GEBCO 2026 日本子集（`pwa/assets/source/bathy/gebco_2026/gebco_2026_118_150_22_50.nc`，15 弧秒，6720×7680）做了一次**变程分析（variogram / RMS-at-scale）**，把猜测换成实测。

**方法**：
1. 取两块窗口——中央本州山地（138.0°E, 36.1°N，94% 陆地）与西太平洋深海（145.0°E, 32.0°N，100% 海洋）。
2. 对每个窗口做多尺度高斯低通，残差 RMS 即"该尺度的粗糙度"：`rms(S) = std(patch − lowpass(S))`，S ∈ {0.4, 0.8, 1.5, 3, 6, 12} km。
3. 拟合 `log rms = c + H·log S` 得 Hurst 指数 H；外推至 40 m（≈80m 周期 band）得该尺度 RMS，×1.5 作为峰值振幅（= `baseAmplitude`）。

**实测结果**：

| 窗口 | Hurst H | RMS@40m | baseAmplitude(峰值) | GEBCO km 尺度 std（参照） |
|---|---|---|---|---|
| 本州山地（陆地） | 0.61 | 11.3 m | **≈17 m** | 116 m |
| 西太平洋深海（海底） | 0.86 | 1.3 m | **≈1.9 m** | 30 m |

**结论与修正**：
- 陆地 `baseAmplitude` 实测 ≈17 m，落在原猜测 15–45 m 的低段 → **原猜测偏宽，以实测为基准，山地可上探至 ~30 m（更陡地形 H 更大），平原下探至 ~10 m**。
- 海床 `baseAmplitude` 实测 ≈1.9 m，**低于原猜测 3–12 m 的低端** → 原海床猜测偏慷慨，修正为 2–6 m（深海 ~2 m，近岸沙波纹可达 4–6 m）。
- GEBCO 在 km 尺度的真实 std（陆地 116 m / 海床 30 m）远大于其 10–80m 尺度应有的振幅（17 m / 1.9 m），说明 **fBm 叠加层只需补足 GEBCO 平滑掉的高频细节，绝不可覆盖其大尺度轮廓**——与"加性/坡度遮罩"混合策略一致。

**谱图**：`figures/terrain_spectrum_gebco_japan.png`（log-log 粗糙度 vs 尺度，含 H 拟合线与 40m 外推标记）。机器可读摘要：`figures/terrain_calib_summary.json`。

**诚实的局限（留给 Phase 7 验证）**：此处是**外推**而非 30m 参考数据的直接测量——GEBCO 在 10–80m 尺度无真实信息，H 由 km 尺度拟合外推得到。Phase 7 落地时应以 **Copernicus GLO-30 / SRTM 30m** 参考数据做交叉校验（项目 `source_status.md` 已记录 GLO-30 为待下载项），确认 10–80m 振幅与本次外推一致。
...
|---|---|---|---|---|---|
| 海岸/海滩 | 10–18 m | 2–6 m | 2 km | 25 km | 细腻沙纹、潮间带纹理（海底沙波纹可达 4–6 m） |
| 山地/峡谷 | 18–30 m | N/A | 4 km | 45 km | 岩壁粗糙、碎石坡（实测山地峰值 ~17 m，更陡地形上探 30 m） |
| 平原/草原 | 10–15 m | 2–5 m | 3 km | 30 km | 微地形起伏（平原低于山地基准） |
| 极地冰盖 | 12–22 m | 2–6 m | 5 km | 40 km | 冰裂隙、雪丘 |
...
*文档版本: v1.1 · 2026-07-19（v1.1: 基于 GEBCO 2026 日本子集实测校准 `baseAmplitude`，新增"实测校准"章节 + 谱图；VS/FS 分工伪代码见"应用方式"节）*
