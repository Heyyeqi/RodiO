...
17. **[#57](https://github.com/Heyyeqi/RodiO/issues/57) 全球海水颜色物理准确性** — Step 0可行性调研已完成并独立验证通过：首选Copernicus GlobColour BGC L4（CHL/SPM/CDM/Kd490同一NetCDF，4km月度全球，Plate Carrée投影跟`dayTexture`同投影不需重投影），备选NASA Ocean Color；`water_params_reference.js`公式零改造。**当前最前面待开工**：阶段A数据获取+烘焙脚本（~3-4天出第一张可见全球水色纹理），完整集成预估2-3周/4阶段。
...
| [#57](https://github.com/Heyyeqi/RodiO/issues/57) | 全球海水颜色物理准确性（水质科学管线扩展到全球） | Earth Visual Foundation Formal Closure | P1 | Step 0调研完成，阶段A（数据获取+烘焙）待开工 |
