...
17. **[#57](https://github.com/Heyyeqi/RodiO/issues/57) 全球海水颜色物理准确性** — Step 0（可行性调研）+ 阶段A（真实全球水色纹理，Copernicus GlobColour BGC L4 2024-06真实下载+烘焙，`global_watercolor_rgba.png` 4096×2048）均已完成并独立验证通过（重跑全链路复现5个验证站位色值精确一致）。**当前最前面待开工**：阶段B，接入`earth3d.js`渲染（跟`dayTexture`同投影混合）。
...
| [#57](https://github.com/Heyyeqi/RodiO/issues/57) | 全球海水颜色物理准确性（水质科学管线扩展到全球） | Earth Visual Foundation Formal Closure | P1 | Step 0+阶段A完成（真实纹理已产出），阶段B（接入渲染）待开工 |
