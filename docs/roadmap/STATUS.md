...
17. **[#57](https://github.com/Heyyeqi/RodiO/issues/57) 全球海水颜色物理准确性** — Step 0+阶段A（真实数据+烘焙纹理）已完成并独立验证通过。阶段B（接入`earth3d.js`渲染）技术接入方式已验证可行，但**颜色质感未过关已撤回**：`deriveWaterParams()`物理反射率色值饱和度过高，像科研热力图不像自然照片色，跟陆地贴图/项目"克制真实"调性不符，代码已用`git checkout`完全还原。**当前最前面待开工**：色调映射/降饱和设计（参考真实卫星true color合成图色彩分布校准），调对了再重新接入（接入手法已验证不用重做）。
