...
## 2026-07-17 HZ播放页布局精简：顶栏合并、控件分隔线移除、farOrbit 居中修正

### 做了什么
- 删除独立的 `.topbar`（"RodiO" 文字行 + 底部分隔线），品牌名合并进原 `ON AIR` 位置（文案改为 `RodiO`，圆点和其余样式不变），`.hero-meta`（时间/城市/RodiO）随之上移贴近顶部。
- `.hero-center`（歌名/歌手文字块）由 `justify-content: center` 改为 `flex-start` + `padding-top: 28px`，文字整体上移，不再居中偏下。
- 删除 `.controls` 按钮组的上下装饰分隔线（`border-top`/`border-bottom`）。
- `.page1-footer`（进入第二页的点击/上滑热区）去掉边框线和 `.page1-footer-bar` 视觉横条，只保留下方已有的两个页面指示圆点作为唯一视觉提示，点击跳转 page2 的功能不变。
- 修正 `farOrbit` 构图的垂直居中：新增 `lookAtY: -1.6`。此前误用近景构图（z≈3.7-4.2）的偏移量级（`-0.4`）套用到 farOrbit（z≈25），视觉上完全无感；实测 `-3.0` 过头、`-1.6` 接近目标位置，已用本地预览截图验证。
...
### 遗留问题
- 第二阶段（进度条淡出、播放控件隐藏、极简播放态、tap-to-reveal 手势）已讨论出方案草案但未实施。
- farOrbit 的 `-1.6` 是单次截图核对的结果，未跑完整播放周期观察其他构图切换时是否协调。

## 2026-07-18 HZ定位极区三角形深色贴片

### 做了什么
- 对用户提供的清晨地球截图进行图层与几何特征分析，确认异常边缘对应球面 atlas 瓦片边界，不是连续的昼夜终止线、云层阴影或正常光照。
- 本地以相同纵向视口启动页面并读取 WebGL 日志；当前约 31°N、121°E 的目标视角只请求了 `16k` atlas 的 `x=0、2、3` 两行，缺少 `x=1`，而该格正对应北美、格陵兰和北大西洋区域。
- 确认 `computeVisibleTileSet()` 现有极区保护只在 `|lat| >= 50` 时加载整行；镜头目标纬度低于阈值时，画面仍可能露出北极，导致保护漏判。
- 对比缺格回退图 `pwa/assets/earth_day_8k.jpg` 与生产瓦片：回退图海洋接近黑色，生产瓦片为亮蓝色；矩形缺格在球面北极收敛后正好表现为带地理细节的深色三角/风筝形贴片。
- 形成后续修复方案：以真实相机投影判断南北极是否可见并补齐对应 atlas 整行；将 bootstrap 换成与生产瓦片同调性的低清底图；bootstrap 重绘后恢复已缓存的高清格；补充瓦片覆盖、失败降级和 RDL inspect 残留回归验证。

### 改动文件
- `devlog.md`

### 遗留问题
- 本轮按“分析并确认修复路径”范围未修改 `pwa/earth3d.js`；需下一轮实施并用同一构图验证三角形消失。
- RDL regional overlay 仍建议作为次级防线验证，但当前截图的主因证据指向 `16k` atlas 顶行漏载与异色 bootstrap 回退。

## 2026-07-18 HZ修复极区瓦片接缝并完成全模式镜头审计

### 做了什么
- 将极区瓦片保护从“目标纬度超过 50°”改为真实相机投影判定：只要南极或北极进入画面，就补齐对应 atlas 整行；构图切换预取则提前补齐目标半球极区，避免过渡中短暂缺格。
- 将 atlas bootstrap 底图替换为与生产瓦片同一色调管线的 `d5z_b_8192x4096.jpg`，并在 bootstrap 异步完成重绘后恢复所有已激活高清瓦片，消除深色回退格及高清格被晚到的底图覆盖两类接缝。
- 关闭强度为零的近共面 ocean tint 球体，避免透明深度面在极端角度产生菱形/三角形分面；切换主题、审计角度、预设和构图时统一清理 RDL inspect overlay，避免调试区域残留到正常画面。
- 更新 Earth 脚本缓存版本至 `polar-atlas-guard-v50`。
- 在本地 WebGL 页面完成 12 个时段主题 × 8 个相机预设（96 个组合）审计，并补测 10 个 Camera Grammar 构图的日间/深夜表现、切镜中间帧与稳定帧；北极、南极、地平线、低轨、远景和深空均未再出现三角缺格或持续黑屏。
- 运行 8 项 Node 测试、JavaScript 语法检查、diff whitespace 检查；bootstrap 资源经本地 HTTP 验证返回 200。浏览器仅有与本次无关的播放 watchdog 告警。
...
### 遗留问题
- 本次未部署 Railway；上线后仍建议在用户实际设备刷新缓存，再以原始截图构图做一次最终肉眼确认。

## 2026-07-18 HZ修复近景构图纹理模糊

### 做了什么
- 复现用户截图，确认异常对应 `cityFocus`：旧参数只让约 180 个 16K atlas 源像素覆盖 Retina 高度画面，地表和云层被放大数倍；不是瓦片加载失败或滤镜设置错误。
- 审计 8 个 Camera Presets 和 10 个 Camera Grammar 构图；`cityFocus` 为明显失真项，`lowOrbit`、`horizon`、`horizonSkim` 在高 DPI 设备上接近纹理密度下限，其余角度清晰度正常。
- 按约 900 个垂直源纹素的最低预算重新约束四个近景参数，保留贴近地表的构图感，同时避免超出 16K 全球纹理的可恢复细节范围。
- 为 Camera Grammar 增加过渡期 LOD 固定策略：靠近地球时提前升到目标高分辨率，远离时等地球缩小后再降级，避免过渡中在 4K/8K/16K 间切换造成短暂发软。
- 即时相机预设现在会取消尚未完成的 Grammar 过渡及 LOD 固定，避免下一帧又被旧过渡拉回错误距离或分辨率。
- 更新 Earth 脚本缓存版本至 `close-view-clarity-v51`；用户在实际 Chrome 中确认修复后的近景清晰度符合预期，控制台显示稳定使用 16K atlas。
- 修复后重新回归全部 8 个预设、关键动态构图稳定帧与靠近/远离中间帧；运行 8 项 Node 测试、JavaScript 语法检查和 diff whitespace 检查，最终本地页面无 warning/error。
