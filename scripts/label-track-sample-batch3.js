#!/usr/bin/env node
/**
 * label-track-sample-batch3.js — Phase 1 Step 3 (batch3): Label 3000 NEW tracks
 *
 * Usage: DEEPSEEK_API_KEY=sk-xxx node scripts/label-track-sample-batch3.js
 *
 * Samples 3000 tracks NOT already labeled in batch1+batch2 (excluded via CSV track_key),
 * enforces merged per-artist quota (batch1 + batch2 + new count <= 3),
 * sends each to DeepSeek using the same v2 SYSTEM_PROMPT + validateLabels hard checks,
 * writes results into track_profile (INSERT OR REPLACE) under label_source='deepseek_sample_batch3',
 * and generates output/track_label_review_batch3.csv.
 */
...
const LABEL_SOURCE = 'deepseek_sample_batch3'
const ROOT = path.join(__dirname, '..')
const DB_PATH = path.join(ROOT, 'db', 'state.db')
const REVIEW_CSV = path.join(ROOT, 'output', 'track_label_review_batch3.csv')
const BATCH1_CSV = path.join(ROOT, 'output', 'track_label_review.csv')
const BATCH2_CSV = path.join(ROOT, 'output', 'track_label_review_batch2.csv')
...
// ── Sampling ─────────────────────────────────────────────────────────
// initialQuota: Map<artistKey, number> — 已有配额（如 batch1 已用），合并计算后上限仍为 maxPerArtist
function sampleWithArtistLimit(pool, size, maxPerArtist, initialQuota = new Map()) {
  ...
  const artistCounts = new Map(initialQuota)
  const selected = []
...
// ── Main loop ────────────────────────────────────────────────────────
async function main() {
  console.log('[label-track-sample-batch3] Phase 1 Step 3 (batch3) — 抽样 3000 首新打标')
  console.log(`[label-track-sample-batch3] 模型: deepseek-v4-flash`)

  // Open DB (needed to write results)
  const db = new Database(DB_PATH)
  db.pragma('journal_mode = WAL')

  // 1) 从 batch1 + batch2 的 CSV 提取已标注 track_key，从候选池排除
  //    track_key = normalizeSongKey(name)::normalizeArtistKey(artist)
  function loadCsvTrackKeysAndQuota(csvPath) {
    const content = fs.readFileSync(csvPath, 'utf8')
    const records = csv.parse(content, { columns: true, skip_empty_lines: true, relax_quotes: true })
    const keys = new Set()
    const quota = new Map()
    for (const rec of records) {
      ...
      const tk = `${normalizeSongKey(name)}::${normalizeArtistKey(artist)}`
      keys.add(tk)
      const ak = String(artist || '').trim().toLowerCase()
      if (ak) quota.set(ak, (quota.get(ak) || 0) + 1)
    }
    return { keys, quota }
  }

  const b1 = loadCsvTrackKeysAndQuota(BATCH1_CSV)
  const b2 = loadCsvTrackKeysAndQuota(BATCH2_CSV)
  const excludedKeys = new Set([...b1.keys, ...b2.keys])
  console.log(`[batch3] batch1 已标注: ${b1.keys.size} 首，batch2 已标注: ${b2.keys.size} 首`)
  console.log(`[batch3] 合并排除 track_key: ${excludedKeys.size} 首`)

  // 2) 合并 batch1 + batch2 每个艺人的已用配额（三批合并上限 MAX_PER_ARTIST）
  const mergedQuota = new Map()
  for (const [ak, c] of b1.quota) mergedQuota.set(ak, (mergedQuota.get(ak) || 0) + c)
  for (const [ak, c] of b2.quota) mergedQuota.set(ak, (mergedQuota.get(ak) || 0) + c)
  const quotaArtists = [...mergedQuota.entries()].sort((a, b) => b[1] - a[1])
  console.log(`[batch3] 合并涉及艺人 ${mergedQuota.size} 个，配额 Top10:`,
    quotaArtists.slice(0, 10).map(([a, c]) => `${a}=${c}`).join(', '))

  // Load pool & sample (排除 batch1+batch2 已标注)
  const pool = loadPool()
  console.log(`[batch3] 曲库总量: ${pool.length}`)
  const poolFiltered = pool.filter((song) => {
    const tk = `${normalizeSongKey(song.name)}::${normalizeArtistKey(song.artist)}`
    return !excludedKeys.has(tk)
  })
  console.log(`[batch3] 排除 batch1+batch2 后候选池: ${poolFiltered.length}`)

  const sample = sampleWithArtistLimit(poolFiltered, SAMPLE_SIZE, MAX_PER_ARTIST, mergedQuota)
  console.log(`[batch3] 抽样: ${sample.length} 首（每艺人合并配额上限 ${MAX_PER_ARTIST}）`)
