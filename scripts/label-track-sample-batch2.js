#!/usr/bin/env node
/**
 * label-track-sample-batch2.js — Phase 1 Step 2 (batch2): Label 800 NEW tracks
 *
 * Usage: DEEPSEEK_API_KEY=sk-xxx node scripts/label-track-sample-batch2.js
 *
 * Samples 800 tracks NOT already labeled in batch1 (label_source='deepseek_sample_batch1'),
 * enforces merged per-artist quota (existing batch1 count + new count <= 3),
 * sends each to DeepSeek using the same v2 SYSTEM_PROMPT + validateLabels hard checks,
 * writes results into track_profile (INSERT OR REPLACE) under label_source='deepseek_sample_batch2',
 * and generates output/track_label_review_batch2.csv.
 */
...
const LABEL_SOURCE = 'deepseek_sample_batch2'
const BATCH1_SOURCE = 'deepseek_sample_batch1'
const ROOT = path.join(__dirname, '..')
const DB_PATH = path.join(ROOT, 'db', 'state.db')
const REVIEW_CSV = path.join(ROOT, 'output', 'track_label_review_batch2.csv')
...
// ── Main loop ────────────────────────────────────────────────────────
async function main() {
  console.log('[label-track-sample-batch2] Phase 1 Step 2 (batch2) — 抽样 800 首新打标')
  console.log(`[label-track-sample-batch2] 模型: deepseek-v4-flash`)

  // Open DB (needed to read batch1 existing keys + per-artist quota)
  const db = new Database(DB_PATH)
  db.pragma('journal_mode = WAL')

  // 1) 读取 batch1 已标注的 track_key，从候选池排除
  const batch1Rows = db
    .prepare(`SELECT track_key, canonical_artist FROM track_profile WHERE label_source = ?`)
    .all(BATCH1_SOURCE)
  const batch1Keys = new Set(batch1Rows.map((r) => r.track_key))
  console.log(`[batch2] batch1 已标注行数: ${batch1Rows.length}`)

  // 2) 统计 batch1 每个艺人的已用配额（合并配额用）
  const batch1Quota = new Map()
  for (const r of batch1Rows) {
    const ak = String(r.canonical_artist || '').trim().toLowerCase()
    if (!ak) continue
    batch1Quota.set(ak, (batch1Quota.get(ak) || 0) + 1)
  }
  const quotaArtists = [...batch1Quota.entries()].sort((a, b) => b[1] - a[1])
  console.log(`[batch2] batch1 涉及艺人 ${batch1Quota.size} 个，配额 Top10:`,
    quotaArtists.slice(0, 10).map(([a, c]) => `${a}=${c}`).join(', '))

  // Load pool & sample (排除 batch1 已标注)
  const pool = loadPool()
  console.log(`[batch2] 曲库总量: ${pool.length}`)
  const poolFiltered = pool.filter((song) => {
    const tk = `${normalizeSongKey(song.name)}::${normalizeArtistKey(song.artist)}`
    return !batch1Keys.has(tk)
  })
  console.log(`[batch2] 排除 batch1 后候选池: ${poolFiltered.length}`)

  const sample = sampleWithArtistLimit(poolFiltered, SAMPLE_SIZE, MAX_PER_ARTIST, batch1Quota)
  console.log(`[batch2] 抽样: ${sample.length} 首（每艺人合并配额上限 ${MAX_PER_ARTIST}）`)
