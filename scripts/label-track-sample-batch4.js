#!/usr/bin/env node
/**
 * label-track-sample-batch4.js — Phase 1 Step 4 (batch4): Label ALL REMAINING tracks
 *
 * Usage: DEEPSEEK_API_KEY=sk-xxx node scripts/label-track-sample-batch4.js
 *
 * Processes the FULL remaining track library (no sampling, no per-artist quota),
 * excluding any track already labeled in batch1+batch2+batch3 (checked directly via DB
 * by label_source = 'deepseek_sample_batch1/2/3'). This makes the run naturally
 * resumable: a mid-run failure and re-run will automatically skip already-written tracks.
 * Sends each to DeepSeek using the same v2 SYSTEM_PROMPT + validateLabels hard checks,
 * writes results into track_profile (INSERT OR REPLACE) under label_source='deepseek_sample_batch4',
 * and generates output/track_label_review_batch4.csv.
 */
...
const LABEL_SOURCE = 'deepseek_sample_batch4'
const ROOT = path.join(__dirname, '..')
const DB_PATH = path.join(ROOT, 'db', 'state.db')
const REVIEW_CSV = path.join(ROOT, 'output', 'track_label_review_batch4.csv')
...
// ── Main loop ────────────────────────────────────────────────────────
async function main() {
  console.log('[label-track-sample-batch4] Phase 1 Step 4 (batch4) — 全量标注剩余曲库')
  console.log(`[label-track-sample-batch4] 模型: deepseek-v4-flash`)

  // Open DB (needed to read already-labeled keys + write results)
  const db = new Database(DB_PATH)
  db.pragma('journal_mode = WAL')

  // 1) 直接查 DB，收集 batch1/2/3 已标注的 track_key，从候选池排除
  //    天然支持断点续跑：重跑时本批已写入的 track_key 也会被排除
  const labeledRows = db
    .prepare(`SELECT track_key FROM track_profile WHERE label_source IN (?, ?, ?)`)
    .all('deepseek_sample_batch1', 'deepseek_sample_batch2', 'deepseek_sample_batch3')
  const excludedKeys = new Set(labeledRows.map((r) => r.track_key))
  console.log(`[batch4] batch1/2/3 已标注: ${excludedKeys.size} 首`)

  // Load pool & build candidate pool (排除已标注，全量遍历，无抽样、无配额)
  const pool = loadPool()
  console.log(`[batch4] 曲库总量: ${pool.length}`)
  const candidatePool = pool.filter((song) => {
    const tk = `${normalizeSongKey(song.name)}::${normalizeArtistKey(song.artist)}`
    return !excludedKeys.has(tk)
  })
  console.log(`[batch4] 排除已标注后候选池: ${candidatePool.length}`)
  ...
  let idx = 0
  const queue = candidatePool.slice()
  let active = 0
  ...
  async function processNext() {
    ...
    if (trackKeysWritten.has(trackKey)) {
      console.log(`[${songIdx + 1}/${candidatePool.length}] ⏭  SKIP (duplicate key): ${song.name} / ${song.artist}`)
      active--
      return
    }
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (valid) {
          ...
          console.log(`[${songIdx + 1}/${candidatePool.length}] 🔄 RETRY ${attempt + 1}/${MAX_RETRIES}: ${song.name} — ${lastError}`)
        }
      } catch (e) {
        apiFailures++
        lastError = e.message
        console.log(`[${songIdx + 1}/${candidatePool.length}] ❌ API FAIL (attempt ${attempt + 1}): ${song.name} — ${lastError}`)
      }
    }
    ...
    if (label) {
      try {
        ...
        console.log(`[${songIdx + 1}/${candidatePool.length}] ✓  ${song.name} / ${song.artist}`)
      } catch (e) {
        console.log(`[${songIdx + 1}/${candidatePool.length}] ❌ DB WRITE FAIL: ${song.name} — ${e.message}`)
        apiFailures++
      }
    } else {
      console.log(`[${songIdx + 1}/${candidatePool.length}] ✗  DROPPED after ${MAX_RETRIES} retries: ${song.name} — ${lastError}`)
    }
  ...
  // Summary
  console.log(`\n========== SUMMARY ==========`)
  console.log(`候选池:       ${candidatePool.length} 首`)
  console.log(`Processed:    ${written} / ${candidatePool.length} total candidates`)
