(function () {
  ...
  // 移动端判定：LITE_MODE 强制覆盖；视口最短边 ≤ 600 或触摸屏。
  // 阈值 600：iPad mini 竖屏 ~768px，iPhone 15 Pro Max ~430px，Mac 全屏 >> 600。
  function isMobileDevice() {
    if (LITE_MODE) return true
    const minDim = Math.min(window.innerWidth, window.innerHeight)
    if (minDim <= 0) return false   // 窗口尚未完成布局，按桌面处理
  ...
  // 注意：createEarth3D() 中会基于实际窗口尺寸重新计算，覆盖模块级初始值。
  let SPHERE_SEGMENTS = isMobileDevice() ? 64 : 128
  ...
  function createEarth3D() {
    ...
    try {
      ...
      // 基于当前实际窗口尺寸重新计算渲染质量参数。
      // 模块级 let SPHERE_SEGMENTS 在脚本加载时可能用了错误的窗口尺寸，
      // 此时 DOM 已就绪，window.innerWidth/innerHeight 稳定。
      SPHERE_SEGMENTS = isMobileDevice() ? 64 : 128
      ...
      // that same source of truth, so returning home never leaves the rim at 0.
      // Declared here (rather than near its original call site further down)
      // because applyRimGlowThemeConfig() needs it and can run as early as
      // the very first applyTheme() call during boot, before most of
      // createEarth3D()'s later declarations have executed.
      const RIM_GLOW_DISTANCE_SCALE = Object.freeze({ near: 1.0, mid: 0.82, far: 0.55 })
      function getRimGlowDistanceScale(compositionKey, isFarComposition, themeName) {
        if (compositionKey === 'homeGlobe') return RIM_GLOW_DISTANCE_SCALE.near
        // deepSpace is rendered as a tiny globe against a large sky field.
        // Its dedicated per-theme profile controls the energy; applying the
        // generic 0.55 far scale here would make bright daytime limbs vanish.
        if (isDeepSpaceGlow(compositionKey, themeName)) return 1.0
        return isFarComposition ? RIM_GLOW_DISTANCE_SCALE.far : RIM_GLOW_DISTANCE_SCALE.mid
      }
      // homeGlobe's own authored width/extent (0.18 / up to 0.30) reads wider
      // than intended once compared directly against the tiered far views.
      // This scales ONLY the range/width uniforms for homeGlobe — brightness
      // (uHaloStrength etc., via RIM_GLOW_DISTANCE_SCALE.near = 1.0 above)
      // stays untouched. mid/far/deepSpace keep using rimDistanceScale as-is.
      const RIM_GLOW_EXTENT_NEAR_SCALE = 0.75
      // Ordinary compositions previously used a Gaussian width that was wider
      // than (or close to) the allowed halo extent. The halo therefore still
      // carried substantial energy when endFade clipped it, which read as a
      // grey/white band. Keep the authored extent, but let the energy decay to
      // near-zero before the cutoff. deepSpace keeps its dedicated wide profile.
      const RIM_GLOW_FALLOFF_WIDTH_SCALE = 0.45
      function getRimGlowExtentScale(compositionKey, rimDistanceScale) {
        return compositionKey === 'homeGlobe' ? RIM_GLOW_EXTENT_NEAR_SCALE : rimDistanceScale
      }
      // DeepSpace is a distinct scale tier, not a night-only exception. At this
      // distance the globe needs a readable outer atmosphere even in bright
      // themes, but the shell's energy and hue must follow the surface below it.
      // Near/mid compositions keep using their authored Rim Overlay untouched.
      const DEEP_SPACE_GLOW_PROFILES = Object.freeze({
        dawn:           { color: '#dcebf4', colorOuter: '#5c83a2', opacity: 0.19, power: 1.8, powerOuter: 1.3, strengthOuter: 0.24, shellEnabled: false, rimWidth: 0.48, rimExtent: 0.64, coreStrength: 0.19, haloStrength: 0.19, innerStrength: 0.11, glowScale: 1.20, ambientRimLift: 0.025 },
        sunrise:        { color: '#fff0d5', colorOuter: '#bd8661', opacity: 0.18, power: 1.8, powerOuter: 1.3, strengthOuter: 0.23, rimWidth: 0.42, rimExtent: 0.54, coreStrength: 0.28, haloStrength: 0.16, innerStrength: 0.10, glowScale: 1.00, ambientRimLift: 0.00 },
        earlyMorning:   { color: '#edf8ff', colorOuter: '#86b9db', opacity: 0.17, power: 1.8, powerOuter: 1.3, strengthOuter: 0.22, rimWidth: 0.40, rimExtent: 0.52, coreStrength: 0.28, haloStrength: 0.18, innerStrength: 0.10, glowScale: 0.72, ambientRimLift: 0.00 },
        morning:        { color: '#f2f9fc', colorOuter: '#91b9d1', opacity: 0.15, power: 1.9, powerOuter: 1.3, strengthOuter: 0.20, rimWidth: 0.38, rimExtent: 0.50, coreStrength: 0.32, haloStrength: 0.126, innerStrength: 0.12, glowScale: 1.00, ambientRimLift: 0.00 },
        noon:           { color: '#f7fbfd', colorOuter: '#9bbdce', opacity: 0.13, power: 2.0, powerOuter: 1.35, strengthOuter: 0.18, rimWidth: 0.36, rimExtent: 0.48, coreStrength: 0.35, haloStrength: 0.136, innerStrength: 0.14, glowScale: 1.00, ambientRimLift: 0.00 },
        afternoon:      { color: '#f3e8d9', colorOuter: '#8faabc', opacity: 0.15, power: 1.9, powerOuter: 1.3, strengthOuter: 0.20, rimWidth: 0.38, rimExtent: 0.50, coreStrength: 0.34, haloStrength: 0.135, innerStrength: 0.13, glowScale: 1.00, ambientRimLift: 0.00 },
        goldenApproach: { color: '#f5d8b4', colorOuter: '#7b91a4', opacity: 0.18, power: 1.8, powerOuter: 1.3, strengthOuter: 0.23, rimWidth: 0.42, rimExtent: 0.54, coreStrength: 0.37, haloStrength: 0.17, innerStrength: 0.14, glowScale: 1.00, ambientRimLift: 0.00 },
        sunset:         { color: '#ffd3a4', colorOuter: '#547b96', opacity: 0.20, power: 1.7, powerOuter: 1.25, strengthOuter: 0.25, rimWidth: 0.46, rimExtent: 0.58, coreStrength: 0.43, haloStrength: 0.1995, innerStrength: 0.16, glowScale: 1.00, ambientRimLift: 0.00 },
        evening:        { color: '#dceef8', colorOuter: '#4e7799', opacity: 0.23, power: 1.7, powerOuter: 1.2, strengthOuter: 0.29, rimWidth: 0.50, rimExtent: 0.66, coreStrength: 0.07, haloStrength: 0.12, innerStrength: 0.015, glowScale: 1.80, ambientRimLift: 0.05 },
        lateEvening:    { color: '#d7ecf8', colorOuter: '#426f96', opacity: 0.26, power: 1.65, powerOuter: 1.2, strengthOuter: 0.32, rimWidth: 0.54, rimExtent: 0.72, coreStrength: 0.06, haloStrength: 0.14, innerStrength: 0.010, glowScale: 2.20, ambientRimLift: 0.10 },
        night:          { color: '#f7feff', colorOuter: '#9fd7ff', opacity: 0.30, power: 1.6, powerOuter: 1.2, strengthOuter: 0.35, rimWidth: 0.58, rimExtent: 0.80, coreStrength: 0.05, haloStrength: 0.16675, innerStrength: 0.005, glowScale: 2.56, ambientRimLift: 0.18 },
        deepNight:      { color: '#f7feff', colorOuter: '#9fd7ff', opacity: 0.30, power: 1.6, powerOuter: 1.2, strengthOuter: 0.35, rimWidth: 0.58, rimExtent: 0.80, coreStrength: 0.04, haloStrength: 0.15225, innerStrength: 0.004, glowScale: 2.71, ambientRimLift: 0.22 },
      })
      function getDeepSpaceGlowProfile(themeName) {
        return DEEP_SPACE_GLOW_PROFILES[themeName] || DEEP_SPACE_GLOW_PROFILES.night
      }
      function isDeepSpaceGlow(compositionKey, themeName) {
        return compositionKey === 'deepSpace' && Boolean(getDeepSpaceGlowProfile(themeName))
      }
      function resolveRimGlowRange(compositionKey, themeName, authoredWidth = 0.30) {
        if (isDeepSpaceGlow(compositionKey, themeName)) {
          const profile = getDeepSpaceGlowProfile(themeName)
          return { width: profile.rimWidth, extent: profile.rimExtent, farGlowEnabled: 1.0 }
        }
        const isFarComposition = FAR_COMPOSITIONS.has(compositionKey)
        const distanceScale = getRimGlowDistanceScale(compositionKey, isFarComposition, themeName)
        const extentScale = getRimGlowExtentScale(compositionKey, distanceScale)
        return {
          width: Math.min(authoredWidth, 0.30) * extentScale * RIM_GLOW_FALLOFF_WIDTH_SCALE,
          extent: 0.18 * extentScale,
          farGlowEnabled: 0.0,
        }
      }
      ...
      class FrontendTileStreamingManager {
        ...
        redrawAtlasBaseLayer(options = {}) {
          ...
          // 用低清全球底图覆盖所有 LOD 的 atlas，避免高清瓦片尚未到达时
          // 暴露出规则的近黑色空白列。Night 模式会把这些空白列进一步压暗，
          // 于是它们会以“球体上的竖向条纹”形式出现。高清瓦片到达后再逐格覆盖。
          const useBootstrapImage = bootstrapDayAtlasImage
            && bootstrapDayAtlasState === 'ready'
          if (useBootstrapImage) {
            this.atlasContext.imageSmoothingEnabled = true
      ...
      const _emRimOverlayMat = new THREE.ShaderMaterial({
        ...
        uniforms: {
          ...
          uHaloExtent:         { value: 0.18 },
          // Deep-space uses a separate, explicitly enabled profile. Keeping
          // this as a gate prevents its large thumbnail halo from leaking
          // into ordinary compositions during rapid camera/theme changes.
          uFarGlowEnabled:     { value: 0.0 },
          ...
          // Theme-level master gain: early morning stays restrained while
          // night themes retain a readable, continuous globe contour.
          uGlowScale:          { value: 1.0 },
          // Moves the diffuse halo's energy slightly away from the exact
          // silhouette, preventing a high-contrast strip at the edge.
          uHaloEdgeFade:       { value: 0.0 },
          // Very small lower/dark-side fill so the closed halo remains
          // readable around the whole globe without brightening the edge line.
          uAmbientRimLift:     { value: 0.0 },
          uOpacity:            { value: 1.0 },
        ...
        fragmentShader: `
          ...
          uniform float uHaloEdgeFade;
          uniform float uAmbientRimLift;
          uniform float uOpacity;
          // ─── directional sun lobe ────────────────────────────────────────
          ...
          varying vec2 vUv;
          void main() {
            // Round, closed silhouette distance. The former one-sided arc
            // equation could become a vertical light column when the
            // projected globe moved low or became small in the frame.
            vec2 ellipse = vec2(
              (vUv.x - uRimCenter.x) / max(uRimRadius.x, 1e-4),
              (vUv.y - (uRimCenter.y + uRimOffsetY)) / max(uRimRadius.y, 1e-4)
            );
            float edgeDistance = length(ellipse) - 1.0;
            float edgeAA = fwidth(edgeDistance) + 1e-5;
            float outside = smoothstep(-edgeAA, edgeAA, edgeDistance);
            // Keep the extreme deepSpace probe isolated. If a previous
            // composition left a large width behind, non-deepSpace views
            // must still use the ordinary compact halo.
            float farGlow = step(0.5, uFarGlowEnabled);
            // Width controls the softness of the falloff; extent controls
            // how far it is allowed to remain visible. They must not be the
            // same knob: using 36 as width flattened the profile into a
            // bright, artificial bowling-ball ring.
            float compactWidth = min(max(uSkyHaloWidth, 0.01), 0.30);
            float farWidth = min(max(uSkyHaloWidth, 0.01), 1.00);
            float width = mix(compactWidth, farWidth, farGlow);
            float coreWidth = max(width * max(uCoreFraction, 0.08), 0.01);
            float core = exp(-pow(edgeDistance / coreWidth, 2.0)) * uCoreStrength;
            float halo = exp(-pow(edgeDistance / width, 2.0)) * uHaloStrength;
            // Keep a small amount of energy at the silhouette. The previous
            // edge fade pushed the whole halo outside the usable projected
            // area, making small night globes appear to have no atmosphere.
            float haloEdgeFade = uHaloEdgeFade > 0.001
              ? smoothstep(0.0, uHaloEdgeFade, max(edgeDistance, 0.0))
              : 1.0;
            halo *= haloEdgeFade;
            float lowerArc = smoothstep(-0.98, -0.18, ellipse.y);
            halo *= 1.0 + lowerArc * uAmbientRimLift;
            float intensity = (uSoftComposite > 0.5)
              ? 1.0 - (1.0 - core) * (1.0 - halo)
              : max(core, halo);
            float farT = clamp(max(edgeDistance, 0.0) / width, 0.0, 1.0);
            vec3 skyColor = mix(uSkyHaloColorNear, uSkyHaloColorFar, farT);
            // The old fixed end fade capped every halo at roughly 0.18
            // projected-radius outside the globe, so increasing width had no
            // visible effect in deepSpace. Tie the fade to width while keeping
            // a bounded maximum to protect the star field from washout.
            // The non-deepSpace branch used to hardcode the literal 0.18 here,
            // silently ignoring whatever the JS side computed for uHaloExtent
            // on homeGlobe/portraitMarble/farOrbit (their tiering never
            // actually reached the render — only uSkyHaloWidth did, via the
            // Gaussian falloff). Reading uHaloExtent on both branches lets the
            // JS-side per-composition value actually take effect.
            float haloExtent = mix(clamp(uHaloExtent, 0.02, 0.18), clamp(uHaloExtent, 0.18, 0.80), farGlow);
            float endFade = 1.0 - smoothstep(haloExtent * 0.72, haloExtent, max(edgeDistance, 0.0));
            float glowAlpha = outside * endFade * uOpacity;

            // Low-energy solar scattering: a tapered, soft ray from the
            // projected globe center toward the real sun position. It is
            // deliberately separate from the contour ring and capped so it
            // cannot become another bright column.
            vec2 sunOffset = vec2(uSunLobeX, uSunLobeY) - uRimCenter;
            float sunDistance = length(sunOffset);
            vec2 sunAxis = sunOffset / max(sunDistance, 1e-4);
            vec2 pointOffset = vUv - uRimCenter;
            float along = dot(pointOffset, sunAxis);
            float across = abs(pointOffset.x * sunAxis.y - pointOffset.y * sunAxis.x);
            float beamWidth = max(0.075, min(0.16, uSunLobeWidth * 0.7));
            float beam = exp(-pow(across / beamWidth, 2.0));
            beam *= smoothstep(-0.02, 0.10, along);
            beam *= 1.0 - smoothstep(sunDistance * 0.52, sunDistance * 0.92, along);
            float skySide = smoothstep(-0.08, 0.12, edgeDistance);
            beam *= skySide * uSunLobeEnabled * 0.10 * uOpacity * uGlowScale;
            vec3 beamColor = mix(uSunLobeOuterColor, uSunLobeMainColor, 0.45);

            gl_FragColor = vec4(skyColor * intensity * glowAlpha * uGlowScale + beamColor * beam, clamp(glowAlpha + beam, 0.0, 1.0));
            return;
      ...
      const _emInnerVeilMat = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          varying vec2 vUv;
          void main() {
            // Match the outer glow to the same closed projected silhouette.
            // This keeps the surface haze continuous without creating a
            // one-sided vertical strip inside the globe.
            vec2 ellipse = vec2(
              (vUv.x - uRimCenter.x) / max(uRimRadius.x, 1e-4),
              (vUv.y - (uRimCenter.y + uRimOffsetY)) / max(uRimRadius.y, 1e-4)
            );
            float edgeDistance = length(ellipse) - 1.0;
            float edgeAA = fwidth(edgeDistance) + 1e-5;
            float insideMask = 1.0 - smoothstep(-edgeAA, edgeAA, edgeDistance);
            float width = max(uInnerVeilWidth, 0.01);
            float falloff = pow(clamp(1.0 + edgeDistance / width, 0.0, 1.0), max(uInnerVeilFalloff, 0.5));
            float endFade = 1.0 - smoothstep(0.96, 1.12, length(ellipse));
            float veilAlpha = insideMask * falloff * endFade * uInnerVeilStrength * uOpacity * uGlowScale;
            gl_FragColor = vec4(uInnerVeilColor, clamp(veilAlpha, 0.0, 1.0));
            return;
      ...
      function applyRimGlowThemeConfig(rimGlowCfg, themeName) {
        ...
        const deepSpaceProfile = isDeepSpaceGlow(_currentCompositionKey, themeName)
          ? getDeepSpaceGlowProfile(themeName)
          : null
        ...
        // Width/extent must stay tiered by the CURRENT composition, not just
        // by theme. applyTheme() can fire after a composition transition has
        // already settled (e.g. real playback driving a theme change while
        // the camera sits at portraitMarble/farOrbit) — if this simply wrote
        // the raw authored width here, it would clobber the tiering that
        // transitionToComposition() applied and bring back the wide ring on
        // far compositions even though nothing "leaked" from deepSpace.
        const rimRange = resolveRimGlowRange(_currentCompositionKey, themeName, outer?.width ?? 0.30)
        ro.uSkyHaloWidth.value    = rimRange.width
        ro.uHaloExtent.value      = rimRange.extent
        ro.uFarGlowEnabled.value  = rimRange.farGlowEnabled
        ro.uCoreFraction.value    = outer?.coreFraction  ?? 0.43
        ro.uCorePower.value       = outer?.corePower     ?? 9.4
        ro.uCoreStrength.value    = deepSpaceProfile?.coreStrength ?? outer?.coreStrength ?? 0.82
        ro.uTailPower.value       = outer?.tailPower     ?? 1.5
        ro.uHaloStrength.value    = deepSpaceProfile?.haloStrength ?? outer?.haloStrength ?? 0.42
        ro.uSoftComposite.value   = outer?.softComposite ? 1.0 : 0.0
        const isNightAtmosphere = themeName === 'deepNight' || themeName === 'night'
        ro.uGlowScale.value       = deepSpaceProfile?.glowScale ?? (isNightAtmosphere
          ? (themeName === 'deepNight' ? 1.48 : 1.40)
          : themeName === 'earlyMorning' ? 0.55 : 1.0)
        // Keep the silhouette connected in night/far views. The gradient is
        // carried by the wide halo itself, not by deleting its edge samples.
        ro.uHaloEdgeFade.value    = 0.0
        ro.uAmbientRimLift.value  = deepSpaceProfile?.ambientRimLift ?? (isNightAtmosphere
          ? (themeName === 'deepNight' ? 0.22 : 0.18)
          : 0.0)
        ...
        rv.uInnerVeilStrength.value = deepSpaceProfile?.innerStrength ?? inner?.strength ?? 0.36
        rv.uInnerVeilFalloff.value  = inner?.falloff  ?? 1.8
        rv.uGlowScale.value         = themeName === 'earlyMorning' ? 0.60 : 1.0
        // ─── directional sun lobe ──────────────────────────────────────────
        if (sunLobe) {
      ...
      function updateEarlyMorningRimProjection() {
        ...
        // Project the existing directional sun into the same screen-space
        // coordinate system. The second batch uses this only for a very weak
        // solar haze; the continuous globe rim above remains the base layer.
        const sunDir = sunLight.position.clone().normalize()
        const sunDistance = Math.max(8, Math.min(cameraDistance * 0.65, 40))
        const sunPoint = _emRimEarthCenter.clone().addScaledVector(sunDir, sunDistance)
        const sunNdc = sunPoint.project(camera)
        const sunU = sunNdc.x * 0.5 + 0.5
        const sunV = sunNdc.y * 0.5 + 0.5
        const sunVisible = Number.isFinite(sunU) && Number.isFinite(sunV)
          && sunNdc.z > -1.0 && sunNdc.z < 1.0
        const rimUniforms = _emRimOverlayMat.uniforms
        // DeepSpace owns its final gain. Other compositions retain the original
        // theme/distance behavior, so entering or leaving the tiny-globe view
        // cannot leave a night-only multiplier behind.
        const deepSpaceProfile = isDeepSpaceGlow(_currentCompositionKey, currentTheme)
          ? getDeepSpaceGlowProfile(currentTheme)
          : null
        if (deepSpaceProfile) {
          rimUniforms.uGlowScale.value = deepSpaceProfile.glowScale
          rimUniforms.uAmbientRimLift.value = deepSpaceProfile.ambientRimLift
        } else if (currentTheme === 'deepNight' || currentTheme === 'night') {
          const themeGlowBase = currentTheme === 'deepNight' ? 1.48 : 1.40
          const distanceGain = THREE.MathUtils.clamp(1.0 + Math.max(cameraDistance - 8.0, 0.0) * 0.018, 1.0, 1.22)
          rimUniforms.uGlowScale.value = themeGlowBase * distanceGain
          rimUniforms.uAmbientRimLift.value = currentTheme === 'deepNight' ? 0.22 : 0.18
        } else {
          rimUniforms.uGlowScale.value = currentTheme === 'earlyMorning' ? 0.55 : 1.0
          rimUniforms.uAmbientRimLift.value = 0.0
        }
        rimUniforms.uSunLobeEnabled.value = currentTheme === 'earlyMorning' && sunVisible ? 1.0 : 0.0
        if (sunVisible) {
          rimUniforms.uSunLobeX.value = sunU
          rimUniforms.uSunLobeY.value = sunV
        }
      }
      ...
      function updateEarlyMorningGlowMode() {
        ...
        const cfg = usesRimOverlay
          ...
          : null
        // The shared screen-space Rim Overlay is authoritative for these
        // themes. Clear both 3D shells on every update so a previous far-view
        // transition cannot leave a stale atmosphere layer visible.
        const deepSpaceGlow = isDeepSpaceGlow(_currentCompositionKey, currentTheme)
        if (usesRimOverlay && atmosphere2 && !deepSpaceGlow) {
          atmosphere2.visible = false
          if (atmosphere2Material) atmosphere2Material.uniforms.uOpacity.value = 0.0
        }
      ...
      const THEME_VISUAL_CONFIG = {
        dawn: {
          ...
          // Legacy CSS horizon micro-glow is disabled for dawn. Deep-space now
          // gets its complete contour from the shared WebGL far-glow profile;
          // stacking this projected DOM haze on top created a visibly separate
          // outer ring around the already-authored core/halo envelope.
          horizonGlow: {
            enabled: false,
            colorCore:  '#e8ecf0',
            colorMain:  '#9bb8cf',
        ...
        evening: {
          ...
          rimGlow: {
            outer: {
              ...
              // Natural night haze: wider falloff and lower core contrast,
              // so the limb reads as air rather than a drawn blue band.
              coreStrength: 0.13, haloStrength: 0.055,
              width: 0.22, coreFraction: 0.42, tailPower: 1.8,
              corePower: 5.8, softComposite: true, rimOffsetY: 0.0,
            },
            inner: {
              color: '#9EBBC9', strength: 0.026,
              width: 0.070, falloff: 2.8,
            },
          },
        ...
        lateEvening: {
          ...
          rimGlow: {
            outer: {
              ...
              // Twilight remains soft and atmospheric, without a neon edge.
              coreStrength: 0.11, haloStrength: 0.050,
              width: 0.20, coreFraction: 0.44, tailPower: 1.9,
              corePower: 6.0, softComposite: true, rimOffsetY: 0.0,
            },
            inner: {
              color: '#8FAEBD', strength: 0.022,
              width: 0.065, falloff: 2.9,
            },
          },
        ...
        deepNight: {
          ...
          rimGlow: {
            outer: {
              color: '#7899AE', colorNear: '#A8C4D3', colorFar: '#192E41',
              // Halo-first night profile: the contour is only a contact
              // accent while the wider, lower-contrast air mass carries the
              // atmosphere. This hides the geometric band without removing
              // the continuous globe halo.
              coreStrength: 0.040, haloStrength: 0.105,
              width: 0.32, coreFraction: 0.58, tailPower: 1.25,
              corePower: 4.8, softComposite: true, rimOffsetY: 0.0,
            },
            inner: {
              color: '#718B9B', strength: 0.004,
              width: 0.10, falloff: 2.0,
            },
          },
        ...
        night: {
          // ═══════════════════════════════════════════════════════════════
          //  Bridge：lateEvening（21:00）→ night（21:45）→ deepNight（22:30）
          //  Lighter than deepNight：higher nightExposure / saturation / gamma
          //  Same architecture：white mapColor + nightGrade daybaseMode
          // ═══════════════════════════════════════════════════════════════
          themeHour: 21.75,
          texture: {
            map: 'day',
          ...
          rimGlow: {
            outer: {
              color: '#7E9FB5', colorNear: '#B5CFDC', colorFar: '#1E384D',
              // Night keeps a little more contour readability than deepNight,
              // but remains halo-first so the edge does not read as a strip.
              coreStrength: 0.050, haloStrength: 0.115,
              width: 0.32, coreFraction: 0.58, tailPower: 1.25,
              corePower: 4.8, softComposite: true, rimOffsetY: 0.0,
            },
            inner: {
              color: '#7894A4', strength: 0.005,
              width: 0.10, falloff: 2.0,
            },
          },
          lighting: { ambient: 1.0, sun: 0.0, stars: 0.74, cityLightsOpacity: 0.435, cityLightClamp: 0.68 },
          horizonGlow: {
            enabled: false,
            colorCore:  '#cce8f8',
            colorMain:  '#80a8c0',
      ...
      function applyOceanTint(themeKey) {
        ...
        // Night-base themes already grade the ocean in the earth shader. The
      ...
      function applyTheme(themeKey, options = {}) {
        ...
        // The thick 3D Fresnel shell is reserved for the per-theme deepSpace system.
        // Never let it survive a composition/theme switch into home or mid views.
        const deepSpaceGlow = isDeepSpaceGlow(_currentCompositionKey, resolvedTheme)
        if (atmosphere2 && atmosphere2Material && !deepSpaceGlow) {
          atmosphere2.visible = false
          atmosphere2Material.uniforms.uOpacity.value = 0.0
        } else if (atmosphere2 && atmosphere2Material && deepSpaceGlow && !isInFarComposition) {
      ...
      function transitionToComposition(compositionKey, opts = {}, _isSequenceStep = false) {
        ...
        // The 3D Fresnel shell is a deepSpace-only treatment. All other
        // compositions use the shared screen-space rim system, so they must not
        // inherit this shell while transitioning back from a far view.
        const deepSpaceProfile = isDeepSpaceGlow(compositionKey, currentTheme)
          ? getDeepSpaceGlowProfile(currentTheme)
          : null
        // Dawn's small deep-space globe showed the back-face Fresnel shell as
        // a second, pixel-sharp contour beside the screen-space halo. Profiles
        // can opt out of that 3D shell while retaining the shared far-glow
        // envelope and all of its distance/theme controls.
        const useFarAtmosphere2 = Boolean(deepSpaceProfile && deepSpaceProfile.shellEnabled !== false)
        if (useFarAtmosphere2) {
          atmosphere.visible = false
          atmosphere2.visible = true
          atmosphere2Material.uniforms.uColor.value.set(deepSpaceProfile.color)
          atmosphere2Material.uniforms.uColorOuter.value.set(deepSpaceProfile.colorOuter)
        ...
        // atmosphere2 transition target values
        const toAtm2Opacity = useFarAtmosphere2 ? deepSpaceProfile.opacity : 0
        const toAtm2SunInfluence = useFarAtmosphere2 ? 0.08 : 0.85
        const toAtm2Power = useFarAtmosphere2 ? deepSpaceProfile.power : (themeCfg.atmosphere2?.power ?? 5.0)
        const toAtm2PowerOuter = useFarAtmosphere2 ? deepSpaceProfile.powerOuter : (themeCfg.atmosphere2?.powerOuter ?? 3.2)
        const toAtm2StrengthOuter = useFarAtmosphere2 ? deepSpaceProfile.strengthOuter : (themeCfg.atmosphere2?.strengthOuter ?? 0.0)

        // Rim glow distance grammar: use the current theme's authored values as
        // the base, then apply one near/mid/far scale. This keeps all camera
        // compositions in the same visual system and restores the authored
        // near value after returning from a far composition.
        ...
        const rimDistanceScale = getRimGlowDistanceScale(compositionKey, isFarComposition, currentTheme)
        const authoredOuter = themeCfg.rimGlow?.outer || {}
        const authoredInner = themeCfg.rimGlow?.inner || {}
        const authoredSunLobe = themeCfg.rimGlow?.sunLobe || {}
        // DeepSpace owns absolute strengths. Near/mid keep their authored theme
        // values, so a quiet close-view rim cannot accidentally erase the tiny
        // far globe (or make dawn inherit a close-view white core).
        const authoredCore = deepSpaceProfile?.coreStrength ?? authoredOuter.coreStrength ?? fromOuterCore
        const authoredHalo = deepSpaceProfile?.haloStrength ?? authoredOuter.haloStrength ?? fromOuterHalo
        const authoredInnerStrength = deepSpaceProfile?.innerStrength ?? authoredInner.strength ?? fromInnerStrength
        const authoredSunLobeStrength = authoredSunLobe.strength ?? fromSunLobeStrength
        const toSunLobeStrength   = authoredSunLobeStrength * rimDistanceScale
        const toOuterCoreStrength = authoredCore * rimDistanceScale
        const toOuterHaloStrength = authoredHalo * rimDistanceScale
        const toInnerStrength     = authoredInnerStrength * rimDistanceScale

        if (_emRimOverlayMat.uniforms.uSkyHaloWidth) {
          // Non-deepSpace far compositions (farOrbit/portraitMarble/etc.) show
          // the whole limb, not just a close sliver like homeGlobe. The same
          // fixed 0.18/0.30 fraction-of-radius that reads as a subtle edge at
          // homeGlobe becomes a plainly visible ring once the full globe is in
          // frame, even though the values themselves never left "normal"
          // range. Scale width/extent by the same near/mid/far tier that
          // already scales strength, so range shrinks together with
          // brightness instead of being a separate, unscaled knob.
          const rimRange = resolveRimGlowRange(compositionKey, currentTheme, authoredOuter.width ?? 0.30)
          _emRimOverlayMat.uniforms.uSkyHaloWidth.value = rimRange.width
          _emRimOverlayMat.uniforms.uHaloExtent.value = rimRange.extent
          _emRimOverlayMat.uniforms.uFarGlowEnabled.value = rimRange.farGlowEnabled
        }
        ...
        _gramTransition = {
          ...
          toLon: resolvedLon,
          fromSunLobeStrength, toSunLobeStrength,
          fromOuterCore, toOuterCoreStrength,
          fromOuterHalo, toOuterHaloStrength,
          fromInnerStrength, toInnerStrength,
      ...
      function _updateGramTransition() {
        ...
        _emRimOverlayMat.uniforms.uSunLobeStrength.value = _gramTransition.fromSunLobeStrength + (_gramTransition.toSunLobeStrength - _gramTransition.fromSunLobeStrength) * e
        _emRimOverlayMat.uniforms.uCoreStrength.value = _gramTransition.fromOuterCore + (_gramTransition.toOuterCoreStrength - _gramTransition.fromOuterCore) * e
        _emRimOverlayMat.uniforms.uHaloStrength.value = _gramTransition.fromOuterHalo + (_gramTransition.toOuterHaloStrength - _gramTransition.fromOuterHalo) * e
        _emInnerVeilMat.uniforms.uInnerVeilStrength.value = _gramTransition.fromInnerStrength + (_gramTransition.toInnerStrength - _gramTransition.fromInnerStrength) * e
      ...
      // Apply homeGlobe rim boost on initial load so the default view
      // benefits from the glow enhancement without any visible camera movement.
      // Overridden later by cameraGrammarV1 portraitMarble transition (line ~7408)
      // or cameraGrammarAuto auto-pilot; both carry their own rim boost.
      ...
      Object.assign(earth3dApi, {
        ...
        getGramDebug() {
          return {
            ...
            atm2Opacity: atmosphere2Material.uniforms.uOpacity.value,
            atm2Power: atmosphere2Material.uniforms.uPower.value,
            atm2PowerOuter: atmosphere2Material.uniforms.uPowerOuter.value,
            atm2StrengthOuter: atmosphere2Material.uniforms.uStrengthOuter.value,
            atm2SunInfluence: atmosphere2Material.uniforms.uSunInfluence.value,
            atm2Visible: atmosphere2.visible,
            atm2Radius: atmosphere2Material.uniforms.uRadius.value,
          }
        },
        // TEMP runtime audit hook for the homeGlobe/approach wide-halo
        // investigation (2026-07-17). Safe to remove once the bug is closed.
        getRimOverlayDebug() {
          const ro = _emRimOverlayMat.uniforms
          const iv = _emInnerVeilMat.uniforms
          return {
            compositionKey: _currentCompositionKey,
            currentTheme,
            cameraZ: camera ? camera.position.z : null,
            cameraFov: camera ? camera.fov : null,
            atm2Visible: atmosphere2.visible,
            atm2Opacity: atmosphere2Material.uniforms.uOpacity.value,
            uFarGlowEnabled: ro.uFarGlowEnabled.value,
            uSkyHaloWidth: ro.uSkyHaloWidth.value,
            uHaloExtent: ro.uHaloExtent.value,
            uHaloStrength: ro.uHaloStrength.value,
            uCoreStrength: ro.uCoreStrength.value,
            uGlowScale: ro.uGlowScale.value,
            uAmbientRimLift: ro.uAmbientRimLift.value,
            uOpacity: ro.uOpacity.value,
            uRimCenter: { x: ro.uRimCenter.value.x, y: ro.uRimCenter.value.y },
            uRimRadius: { x: ro.uRimRadius.value.x, y: ro.uRimRadius.value.y },
            innerVeilOpacity: iv.uOpacity.value,
            innerVeilStrength: iv.uInnerVeilStrength.value,
            gramTransitionActive: Boolean(_gramTransitionRef),
          }
        },
        setAtmosphereLayerVisible(layerName, visible) {
          if (layerName === 'atmosphere') {
            atmosphere.visible = visible
            console.log(`[earth3d] atmosphere.visible = ${visible}`)
          } else if (layerName === 'atmosphere2') {
            atmosphere2.visible = visible
            console.log(`[earth3d] atmosphere2.visible = ${visible}`)
          } else {
            console.warn(`[earth3d] setAtmosphereLayerVisible: unknown layer '${layerName}'`)
          }
        },
        getAtmosphereLayerState(layerName) {
          if (layerName === 'atmosphere') {
            return {
              visible: atmosphere.visible,
              uOpacity: atmosphereMaterial.uniforms.uOpacity.value,
              uPower: atmosphereMaterial.uniforms.uPower.value,
              uPowerOuter: atmosphereMaterial.uniforms.uPowerOuter.value,
              uStrengthOuter: atmosphereMaterial.uniforms.uStrengthOuter.value,
              uSunInfluence: atmosphereMaterial.uniforms.uSunInfluence.value,
            }
          } else if (layerName === 'atmosphere2') {
            return {
              visible: atmosphere2.visible,
              uOpacity: atmosphere2Material.uniforms.uOpacity.value,
              uPower: atmosphere2Material.uniforms.uPower.value,
              uPowerOuter: atmosphere2Material.uniforms.uPowerOuter.value,
              uStrengthOuter: atmosphere2Material.uniforms.uStrengthOuter.value,
              uSunInfluence: atmosphere2Material.uniforms.uSunInfluence.value,
              uRadius: atmosphere2Material.uniforms.uRadius.value,
            }
          } else {
            console.warn(`[earth3d] getAtmosphereLayerState: unknown layer '${layerName}'`)
            return null
          }
        },
        ...
        getAtmosphere2Mat() {
          return atmosphere2Material || null
        },
        getCameraDebugInfo() {
          return {
